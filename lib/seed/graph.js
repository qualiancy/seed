...
Graph.prototype.set = function (address, attributes, opts) {
 ...
 * # .get(address)
 *
 * Get model as a specific address in the form of `/type/_id`.
 *
 * @param {String} address in the form `/type/_id`
 * @returns {Model} at address
 * @api public
 * @name .get()
 */
...
Graph.prototype.fetch = function (type, query, callback) {
  ...
  var success = function (data) {
    data.forEach(function(attrs) {
      self.set('/' + type + '/' + attrs.id, attrs, { dirty: false });
    });
