/*!
 * drip - model constructor / prototype
 ...
 * # flag
 * 
 * Flags are key:values that represent the models state but are not syncronized with the
 * server.
 * 
 ...
 * # set
 * 
 * Set the value of a set of attributes. Attributes will be merged with existing attributes.
 * 
 * #### Options
 * 
 ...
 * # save
 * 
 * Save the current attributes of the model to the storage engine. Can execute a `callback`
 * on completion. The `this` context will be the model that was just saved.
 * 
 * Will send a `create` action to server if flag#new is true, or if the model doesn't have
 * an id. Will send an `update` action otherwise.
 * 
 * #### Options
 * 
...
Model.prototype.save = function (opts, callback) {
  // TODO: action should also be based on id
  var self = this,
  ...
  var sync = this.storage.sync(action, this);
...
Model.prototype.fetch = function (opts, callback) {
  ...
  var sync = this.storage.sync('read', this);
  sync.then(success, failure);
};
...
Model.prototype.destroy = function (opts, callback) {
  // TODO: silent doesn't do anything
  ...
  var sync = this.storage.sync('remove', this);
  sync.then(success, failure);
};
