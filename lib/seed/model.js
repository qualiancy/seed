...
Model.prototype.save = function (opts, cb) {
  ...
  if (!opts.force && this.schema && !this.schema.validate(this._attributes))
    return cb(new SeedError('Attributes not valid', { code: 'ENOTVALID' }));
...
function storeSuccess (opts, cb) {
  ...
  return function success (data) {
    if (!data)
      return cb(new SeedError('Model#fetch returned no data', { code: 'ENOTFOUND' }));
    ...
    if (!valid) cb(new SeedError('Attributes not valid', { code: 'ENOTVALID' }));
...
Model.prototype.loadRef = function (ref, cb) {
  ...
  if (!this.store)
    return cb(new SeedError('No storage defined.', { code: 'ENOSTORE' }));
  if (this.get(ref) instanceof Model)
    return cb(null);
  ...
  if (!vert || !vert.$id || !vert.$ref)
    return cb(new SeedError('Reference undefined.', { code: 'ENODBREF' }));
  ...
  if (!meta)
    return cb(new SeedError('Missing type constructor.', { code: 'ENOMODELDEF' }));
