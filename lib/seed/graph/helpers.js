...
function pullEdges (query, cb) {
  ...
  function parseEdges (data) {
    for (var i = 0; i < data.length; i++) {
      var rawEdge = data[i]
        , vertX = (self.has(rawEdge.x.$ref, rawEdge.x.$id))
          ? self.get(rawEdge.x.$ref, rawEdge.x.$id)
          : rawEdge.x
        , vertY = (self.has(rawEdge.y.$ref, rawEdge.y.$id))
          ? self.get(rawEdge.y.$ref, rawEdge.y.$id)
          : rawEdge.y
