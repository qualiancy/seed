...
Model.extend = function (type, proto, klass) {
  if (!tea.isString(type)) {
    klass = proto; proto = type; type = 'model'; }
  ...
  var child = tea.inherits(this, proto, klass);
  child.extend = this.extend;
  return child;
};

tea.merge(Model.prototype, drip.prototype);
...
Model.prototype.set = function (attrs, opts) {
  var old_props = tea.merge({}, this._attributes);
  tea.merge(this._attributes, attrs);
  ...
  if (!opts.silent) {
    ...
    for (var p in attrs) {
      if (this._attributes[p] != old_props[p]) {
        this.emit('change:' + p, { 
          attribute: p, 
...
Model.prototype.save = function (opts, callback) {
  var self = this
    , store = this.parent.store || this.store || null;
...
Model.prototype.fetch = function (opts, callback) {
  ...
  var success = function (data) {
    ...
    self.flag('new', false);
    self.flag('changed', false);
    if (tea.isFunction(callback)) callback.call(self, null);
  };
  
  var failure = function (err) {
    if (tea.isFunction(callback)) callback.call(self, err);
  };
...
Model.prototype.destroy = function (opts, callback) {
  ...
  var success = function (data) {
    if (self.parent.del) self.parent.del('/' + self.type + '/' + self.id);
    if (!opts.silent) {
