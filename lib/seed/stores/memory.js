...
var MemoryStore = module.exports = Store.extend({
  ...
  , get: function (seed) {
      ...
      process.nextTick(function() {
        ...
        if (!group) {
          promise.reject({ code: 3, message: 'seed type doesn\'t exist in store' });
          return;
        }
  ...
  , destroy: function (seed) {
      ...
      process.nextTick(function() {
        ...
        if (!group) return promise.reject({ code: 3, message: 'seed type doesn\'t exist in store' })
        if (!group.get(id)) return promise.reject({ code: 3, message: 'seed doesn\'t exist in store' });
