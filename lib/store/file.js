...
function FileStore (_path, options) {
  ...
  if (!path.exists(_path)) throw new Error('Seed FileStore path does not exist.');
  options = options || {};
  this.path = _path;
...
FileStore.prototype.set = function (seed) {
  ...
  var filename = seed.path + '.' + id
    , file = path.join(this.path, filename + '.json')
    , data = JSON.stringify(seed.data);
  
  fs.writeFile(file, data, function (err) {
    if (err) {
      promise.reject({code: 0, message: 'An unkown error occurred'});
    } else {
