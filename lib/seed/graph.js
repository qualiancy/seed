...
Graph.extend = function (proto, klass) {
  var opts = {};
  klass = klass || {};
  proto = proto || {};
...
var Relation = Model.extend('relation', {
  initialize: function (opts, x, y, r) {
    this.flag('x', x);
    this.flag('y', y);
    this.flag('relation', r);

    Object.defineProperty(this, 'definition',
      { get: function () {
          var def = {};
          def.x = this.flag('x').id;
          def.y = this.flag('y').id;
          def.relation = this.flag('relation');
          def.attributes = _.merge({}, this._attributes);
        }
    });
  }
});

Graph.prototype.relate = function (nodeX, nodeY, relation, recip) {
  var rel = new Relation({}, {}, nodeX, nodeY, relation)
    , rel_key = relation + ':' + nodeX.id + ':' + nodeY.id;
  this._relations.set(rel_key, rel);

  if (recip) {
    var rel = new Relation({}, {}, nodeY, nodeX, relation)
      , rel_key = relation + ':' + nodeY.id + ':' + nodeX.id;
    this._relations.set(rel_key, rel);
  }
};

Graph.prototype.unrelate = function (nodeX, nodeY, relation, recip) {
