/*!
 * Seed :: Filter - MongoDB style array filtering
...
var Filter = exports.Filter = function Filter(query) {
  this.query = query;
  this.stack = _.parse(query);
}
...
Filter.prototype = {
  ...
  test: function(data, opts) {
    return _.testFilter(data, this.stack);
  },
  ...
  subset: function(data) {
    ...
    for (var di = 0; di < data.length; di++) {
      datum = data[di];
      if (_.testFilter(datum, this.stack)) res.push(datum);
    }
  ...
  pass: function(data) {
    ...
    for (var di = 0; di < data.length; di++) {
      datum = data[di];
      res.push(_.testFilter(datum, this.stack));
    }
  ...
  index: function(data) {
    ...
    for (var di = 0; di < data.length; di++) {
      datum = data[di];
      if (_.testFilter(datum, this.stack)) res.push(di);
    }
