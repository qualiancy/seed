...
function getEdge (vertX, vertY, rel) {
  var x =
...
function pullEdges (vert, rel, cb) {
  // base checks and reassociation
  if (!this.store)
    return cb(new SeedError('No storage defined', { code: 'ENOSTORE' }));
  ...
  var query = {
    $and:
      [ { $or:
        [ { 'x.$id': vert.id, 'x.$rel': vert.type }
        , { 'y.$id': vert.id, 'y.$rel': vert.type } ] }
      ]
  };
  ...
  function parseEdges (data) {
    for (var i = 0; i < data.length; i++) {
      var rawEdge = data[i]
        , vertX = (self.has(rawEdge.x.$rel, rawEdge.x.$id))
          ? self.get(rawEdge.x.$rel, rawEdge.x.$id)
          : rawEdge.x
        , vertY = (self.has(rawEdge.y.$rel, rawEdge.y.$id))
          ? self.get(rawEdge.y.$rel, rawEdge.y.$id)
          : rawEdge.y
        , edge = getEdge.call(self, vertX, vertY, rawEdge.rel);

      if (!edge) {
        var edge = buildEdge.call(self, vertX, vertY, rawEdge.rel, rawEdge.attributes);
    ...
    refreshLookups.call(self, self._edges);
    cb(null);
  }
...
Graph.prototype.unrelate = function (vertX, vertY, rel) {
  ...
  var edge = getEdge.call(this, vertX, vertY, rel);
...
Graph.prototype.push = function (opts, callback) {
  ...
  queue.push(function (next) {
    refreshLookups.call(self, self._vertices);
    next();
  });
